...
import { enviarAvisosCadastro, LIMITE_AVISOS_HORA } from "../_shared/avisosCadastro.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // (send-email) e x-cron-secret (notify-dispatch). Os 3 envios moram em
    // `_shared/avisosCadastro.ts`, testados com fetch falso.
    ...
    // Mensagem fixa: um `throw` com texto que dependa do e-mail viraria oraculo.
    console.error(`[register-customer] erro: ${String(err?.message ?? err)}`);
